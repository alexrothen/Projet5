...
function createProductBlock(item) {
  container.innerHTML = `<section class="row justify-content-center mx-auto col-12 mb-4">
    <div class="col-lg-7 col-md-8 col-sm-12"><img src=${
      item.imageUrl
    } class="img-fluid"></div>
        <aside class="col-lg-4 col-md-4" style="height: fit-content;">
            <ul class="row list-group-flush pl-0">
                <li class="text-left list-group-item col-lg-12">${
                  item.name
                }</li>
                <li class="text-justify list-group-item col-lg-12">${
                  item.description
                }</li>
                ...
                <li class="text-center col-md-12 list-group-item  "><strong>Prix : ${formatPrice(
                  item.price
                )} €</strong></li></ul>
...
function optionLoop(item) {
  for (const option in item.lenses) {
    const itemOptionsSelect = document.getElementById("selectOpt");
...
function sendBtn(item) {
  document.getElementById("cartBtn").addEventListener("click", (e) => {
    e.preventDefault;
    const itemQuantitySelect = document.getElementById("selectQt");
    ...
    if (
      cart.find(
        (item) =>
          item.id === addItemToCart.id && item.options === addItemToCart.options
      )
    ) {
      for (const i in cart) {
        ...
        if (
          ...
          cart[i].options === addItemToCart.options
        ) {
          cart[i].quantity = addItemToCart.quantity;
          ...
          console.log(
            `Quantité mise à jour : ${cart[i].quantity} ${cart[i].name} option ${cart[i].options}`
          );
        }
      }
    }
    // SI LE PRODUIT EST ABSENT DU PANIER...
      ...
      console.log(
        `${addItemToCart.quantity} ${addItemToCart.name} option ${addItemToCart.options} dans le panier`
      );
      Swal.fire({
